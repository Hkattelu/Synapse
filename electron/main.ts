...
import type { OpenDialogOptions, SaveDialogOptions, FileFilter } from 'electron';
...
ipcMain.handle('ipc:fs:open-file', async (evt, options: OpenDialogOptions = {}) => {
...
ipcMain.handle('ipc:fs:open-files', async (evt, options: OpenDialogOptions = {}) => {
  const parent = BrowserWindow.fromWebContents(evt.sender) ?? null;
  const merged: OpenDialogOptions = {
...
ipcMain.handle('ipc:fs:open-directory', async (evt, options: OpenDialogOptions = {}) => {
  const parent = BrowserWindow.fromWebContents(evt.sender) ?? null;
  const merged: OpenDialogOptions = {
...
ipcMain.handle('ipc:fs:read-file', async (_evt, path: string, encoding: string | null = 'utf-8') => {
  if (!path || typeof path !== 'string') throw new Error('Invalid path');
  if (encoding) {
    const data = await fs.readFile(path, { encoding: encoding as BufferEncoding });
...
ipcMain.handle(
  'ipc:fs:write-file',
  async (_evt, path: string, data: string | Uint8Array, options: WriteFileOptions | BufferEncoding | null = {}) => {
    if (!path || typeof path !== 'string') throw new Error('Invalid path');
    await fs.writeFile(path, data as any, options as any);
    return true as const;
  }
);

ipcMain.handle('ipc:fs:show-save-dialog', async (evt, options: SaveDialogOptions = {}) => {
  const parent = BrowserWindow.fromWebContents(evt.sender) ?? null;
