...
import { EDUCATIONAL_TRACKS, getEducationalTrackByNumber } from './educationalTypes';
 ...
 */
function getFallbackTrackForItemType(itemType: TimelineItemType): EducationalTrack {
  ...
  return EDUCATIONAL_TRACKS.find(t => t.id === trackId) || EDUCATIONAL_TRACKS[1]; // Default to Visual
}
...
function detectMigrationConflicts(
  ...
  itemsByTrack.forEach((trackItems, trackNumber) => {
    ...
    const suggestedTracks = new Set(trackAnalysis.map(ta => ta.analysis.suggestedTrack.id));
    ...
    if (suggestedTracks.size > 1) {
      ...
      trackAnalysis.forEach(({ item, analysis }) => {
        const currentEducationalTrack = getEducationalTrackByNumber(trackNumber);
        
        if (currentEducationalTrack && analysis.suggestedTrack.id !== currentEducationalTrack.id) {
          // Find alternative tracks
          const alternatives = EDUCATIONAL_TRACKS.filter(t => 
...
function getContentPurpose(
  ...
  const purposeMap: Record<string, Record<TimelineItemType, 'demonstration' | 'explanation' | 'narration' | 'personal'>> = {
    'code': {
      'code': 'demonstration',
      'video': 'demonstration',
      'audio': 'explanation',
...
export function migrateProjectToEducationalTracks(
  ...
  try {
    ...
    if (conflicts.length > 0 && !migrationConfig.autoResolveConflicts && userDecisions.length === 0) {
      return {
        success: false,
        migratedItems: 0,
        conflicts,
        warnings: [`Found ${conflicts.length} migration conflicts that require user resolution`]
      };
    }
 ...
 */
export function getProjectBackups(projectId: string): Array<{ migrationId: string; timestamp: Date }> {
 ...
 */
export function cleanupProjectBackups(projectId: string, keepCount: number = 5): void {
  const projectBackupsList = Array.from(projectBackups.values())
    .filter(backup => backup.projectId === projectId)
    .sort((a, b) => b.backupTimestamp.getTime() - a.backupTimestamp.getTime());
 ...
 */
export function validateProjectForMigration(project: Project): { canMigrate: boolean; issues: string[] } {
  const issues: string[] = [];
  ...
  const orphanedItems = project.timeline.filter(item => !assetIds.has(item.assetId));
  
  if (orphanedItems.length > 0) {
    issues.push(`Found ${orphanedItems.length} timeline items without corresponding media assets`);
  }

  // Check for unsupported item types
  const supportedTypes = new Set(EDUCATIONAL_TRACKS.flatMap(track => track.allowedContentTypes));
  const unsupportedItems = project.timeline.filter(item => !supportedTypes.has(item.type));
  
  if (unsupportedItems.length > 0) {
    issues.push(`Found ${unsupportedItems.length} timeline items with unsupported types`);
  }
...
export function isProjectMigrated(project: Project): boolean {
  ...
  const hasEducationalMetadata = project.timeline.some(item => 
    'educationalTrack' in item || 'educationalMetadata' in item
  );
...
export function getMigrationStatistics(project: Project): {
  ...
  return {
    ...
    itemsByTrack,
    itemsByType,
    averageConfidence: project.timeline.length > 0 ? totalConfidence / project.timeline.length : 0,
