...
export class KeyframeManager {
  ...
  static addKeyframe(item: TimelineItem, keyframe: Keyframe): TimelineItem {
    ...
    const filteredKeyframes = item.keyframes.filter(k => 
      k.time !== adjustedKeyframe.time || 
      !this.hasOverlappingProperties(k.properties, adjustedKeyframe.properties)
    );

    return {
      ...item,
      keyframes: [...filteredKeyframes, adjustedKeyframe].sort((a, b) => a.time - b.time),
    };
  }
  ...
  static updateKeyframe(
    ...
    return {
      ...
      keyframes: item.keyframes.map(k => 
        k.id === keyframeId 
          ? { ...k, ...updates, time: Math.min(Math.max(0, updates.time ?? k.time), item.duration) }
          : k
  ...
  static getPropertyKeyframes(item: TimelineItem, property: keyof ItemProperties): Keyframe[] {
    return item.keyframes.filter(k => property in k.properties).sort((a, b) => a.time - b.time);
  }
  ...
  static interpolateProperty(
    ...
    const beforeValue = beforeKeyframe.properties[property] ?? item.properties[property];
    const afterValue = afterKeyframe.properties[property] ?? item.properties[property];
    ...
    const progress = timeDiff === 0 ? 0 : (clampedTime - beforeKeyframe.time) / timeDiff;
    const easedProgress = EASING_FUNCTIONS[beforeKeyframe.easing || 'linear'](progress);

    return beforeValue + (afterValue - beforeValue) * easedProgress;
  }

  // Get all interpolated properties at a specific time
  static getInterpolatedProperties(item: TimelineItem, time: number): ItemProperties {
  ...
  static generateKeyframesFromPresets(item: TimelineItem): TimelineItem {
    ...
    item.animations.forEach(animation => {
      ...
      switch (animation.type) {
        case 'entrance':
          ...
          break;
        case 'exit':
          this.generateExitKeyframes(animation, item.duration, generatedKeyframes);
          break;
        case 'emphasis':
          this.generateEmphasisKeyframes(animation, item.duration, generatedKeyframes);
          break;
      }
    });
    ...
    return {
      ...item,
      keyframes: [...item.keyframes, ...generatedKeyframes].sort((a, b) => a.time - b.time),
    };
  }
  ...
  static optimizeKeyframes(item: TimelineItem): TimelineItem {
    ...
    for (let i = 0; i < keyframes.length; i++) {
      ...
      if (Object.keys(current.properties).every(key => 
  ...
  // Validate keyframes
  static validateKeyframes(item: TimelineItem): { isValid: boolean; errors: string[] } {
    const errors: string[] = [];

    item.keyframes.forEach((keyframe, index) => {
      // Check time bounds
      if (keyframe.time < 0 || keyframe.time > item.duration) {
        errors.push(`Keyframe ${index + 1}: Time ${keyframe.time} is outside item duration (0-${item.duration})`);
      }

      // Check property values
      Object.entries(keyframe.properties).forEach(([key, value]) => {
        if (key === 'opacity' && typeof value === 'number' && (value < 0 || value > 1)) {
          errors.push(`Keyframe ${index + 1}: Opacity value ${value} must be between 0 and 1`);
        }
        if (key === 'scale' && typeof value === 'number' && value < 0) {
          errors.push(`Keyframe ${index + 1}: Scale value ${value} cannot be negative`);
        }
      });
    });
  ...
  private static hasOverlappingProperties(props1: Partial<ItemProperties>, props2: Partial<ItemProperties>): boolean {
    const keys1 = Object.keys(props1);
    const keys2 = Object.keys(props2);
    return keys1.some(key => keys2.includes(key));
  }

  private static generateEntranceKeyframes(animation: any, keyframes: Keyframe[]): void {
  ...
  private static generateExitKeyframes(animation: any, itemDuration: number, keyframes: Keyframe[]): void {
  ...
  private static generateEmphasisKeyframes(animation: any, itemDuration: number, keyframes: Keyframe[]): void {
...
export function interpolateValue(
  ...
  const easedProgress = EASING_FUNCTIONS[easing](Math.max(0, Math.min(1, progress)));
  return startValue + (endValue - startValue) * easedProgress;
}

export function getKeyframeAtTime(keyframes: Keyframe[], time: number): Keyframe | null {
...
export function copyKeyframes(keyframes: Keyframe[], timeOffset: number = 0): Keyframe[] {
