...
export function YouTrackFeatures({ 
  ...
  const [detectionResult, setDetectionResult] = useState<TalkingHeadDetectionResult | null>(null);
  ...
  useEffect(() => {
    if (item.properties) {
      ...
      setPipConfig(prev => ({
        ...prev,
        position: props.talkingHeadCorner === 'bottom-left' ? 'bottom-left' :
                 props.talkingHeadCorner === 'top-right' ? 'top-right' :
                 props.talkingHeadCorner === 'top-left' ? 'top-left' : 'bottom-right',
        size: props.talkingHeadSize === 'sm' ? 'small' :
              props.talkingHeadSize === 'lg' ? 'large' : 'medium'
      }));
      ...
      if (props.backgroundRemoval) {
        ...
        setBackgroundOptions({ type: 'blur', blurAmount: props.backgroundBlur });
      }
  ...
  const handleVideoLoaded = async () => {
    ...
    try {
      ...
      if (result.hasFace && result.isOptimal) {
        const optimizedProps = optimizeTalkingHeadSettings(result, item.properties || {});
  ...
  const updatePipConfig = (updates: Partial<PiPConfiguration>) => {
    ...
    const properties: Partial<ItemProperties> = {
      ...
      talkingHeadCorner: newConfig.position === 'bottom-left' ? 'bottom-left' :
                        newConfig.position === 'top-right' ? 'top-right' :
                        newConfig.position === 'top-left' ? 'top-left' : 'bottom-right',
      talkingHeadSize: newConfig.size === 'small' ? 'sm' :
                      newConfig.size === 'large' ? 'lg' : 'md',
  ...
  const pipPosition = getPiPPositionCoordinates(pipConfig.position, containerSize, pipSize);
  ...
  return (
    <div className="you-track-features space-y-6">
          ...
          <div className="space-y-3">
            ...
            {detectionResult && (
              <div className="space-y-2">
                <div className={`p-3 rounded ${detectionResult.hasFace ? 'bg-green-50 border border-green-200' : 'bg-yellow-50 border border-yellow-200'}`}>
                  <div className="flex items-center gap-2 mb-2">
                    <span className={`w-2 h-2 rounded-full ${detectionResult.hasFace ? 'bg-green-500' : 'bg-yellow-500'}`}></span>
                    <span className="font-medium">
                      {detectionResult.hasFace ? 'Face Detected' : 'No Face Detected'}
                    </span>
                    {detectionResult.hasFace && (
                      <span className="text-sm text-gray-600">
                        ({Math.round(detectionResult.faceConfidence)}% confidence)
                      </span>
                    )}
                  </div>
                ...
                {detectionResult.hasFace && (
                  <button
                    onClick={() => {
                      const optimized = optimizeTalkingHeadSettings(detectionResult, item.properties || {});
      ...
      <div className="bg-white rounded-lg border p-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                onChange={(e) => updatePipConfig({ borderRadius: parseInt(e.target.value) })}
                className="w-full"
              />
              <span className="text-xs text-gray-500">{pipConfig.borderRadius}px</span>
            </div>
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => updatePipConfig({ opacity: parseFloat(e.target.value) })}
                className="w-full"
              />
              <span className="text-xs text-gray-500">{Math.round(pipConfig.opacity * 100)}%</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <input
                ...
                onChange={(e) => updatePipConfig({ borderColor: e.target.value })}
        ...
        {/* Preview */}
        <div className="mt-4 p-4 bg-gray-100 rounded relative" style={{ aspectRatio: '16/9' }}>
          ...
          <div
            ...
            style={{
              ...
              boxShadow: pipConfig.shadow ? '0 4px 8px rgba(0,0,0,0.3)' : 'none'
            }}
          >
            You
          </div>
      ...
      <div className="bg-white rounded-lg border p-4">
        ...
        <div className="space-y-3">
          ...
          {backgroundOptions.type === 'blur' && (
            <div>
              ...
              <input
                ...
                onChange={(e) => updateBackgroundOptions({ 
                  ...backgroundOptions, 
          ...
          {backgroundOptions.type === 'greenscreen' && (
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium mb-1">Chroma Key Color</label>
                <input
                  type="color"
              ...
              <div>
                ...
                />
                <span className="text-xs text-gray-500">
                  {Math.round((backgroundOptions.chromaKeyTolerance || 0.3) * 100)}%
                </span>
              </div>
            </div>
          )}
