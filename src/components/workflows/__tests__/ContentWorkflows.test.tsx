...
describe('Content Addition Workflows', () => {
  describe('CodeEditorWorkflow', () => {
    ...
    it('shows code templates by default', () => {
      ...
      expect(screen.getByText('Function Example (JavaScript)')).toBeInTheDocument();
    });
    ...
    it('loads template when clicked', () => {
      ...
      expect(screen.getByDisplayValue('Hello World (JavaScript)')).toBeInTheDocument();
    });
    ...
    it('does not render when closed', () => {
      ...
      expect(screen.queryByText('Add Code to Timeline')).not.toBeInTheDocument();
    });
  });
  ...
  describe('VideoWorkflow', () => {
    ...
    it('shows video configuration when type is selected', () => {
      ...
      expect(screen.getByText('Recording Tips for Screen Recording')).toBeInTheDocument();
    });

    it('does not render when closed', () => {
      render(<VideoWorkflow {...defaultProps} isOpen={false} />);
      
      expect(screen.queryByText('Add Video to Timeline')).not.toBeInTheDocument();
    });
  });
  ...
  describe('AssetsWorkflow', () => {
    ...
    it('shows empty state initially', () => {
      ...
      expect(screen.getByText('Choose a category from the sidebar to upload educational assets')).toBeInTheDocument();
    });

    it('does not render when closed', () => {
      render(<AssetsWorkflow {...defaultProps} isOpen={false} />);
      
      expect(screen.queryByText('Add Educational Assets')).not.toBeInTheDocument();
    });
  });
  ...
  describe('ContextualHelp', () => {
    ...
    it('switches between help tabs', () => {
      ...
      // Default to tips
      expect(screen.getByText('Use syntax highlighting to improve readability')).toBeInTheDocument();
      
      // Switch to best practices
      fireEvent.click(screen.getByText('Best Practices'));
      expect(screen.getByText('Start with simple examples before complex ones')).toBeInTheDocument();
      
      // Switch to examples
      fireEvent.click(screen.getByText('Examples'));
      expect(screen.getByText('Function definitions with clear examples')).toBeInTheDocument();
    });
  });
});
...
describe('Workflow Integration', () => {
  it('provides contextual guidance for educational content creation', () => {
    ...
    expect(screen.getByText(/Create engaging code demonstrations/)).toBeInTheDocument();
    
    rerender(<ContextualHelp type="video" />);
    expect(screen.getByText(/Create professional educational videos/)).toBeInTheDocument();
    
    rerender(<ContextualHelp type="assets" />);
    expect(screen.getByText(/Select and organize visual and audio assets/)).toBeInTheDocument();
  });

  it('supports educational workflow requirements', () => {
    // Test that workflows support the requirements from the spec
    render(<CodeEditorWorkflow isOpen={true} onClose={vi.fn()} onCodeAdded={vi.fn()} />);
