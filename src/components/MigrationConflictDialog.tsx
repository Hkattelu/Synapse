      ...
      <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full mx-4 max-h-[80vh] overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-200">
          ...
          <p className="text-sm text-gray-600 mt-1">
            Some timeline items need manual track assignment. Please review and
            select the appropriate track for each item.
          </p>
        </div>
          ...
          <div className="space-y-6">
            {conflicts.map((conflict, index) => {
              const resolution = resolutions.find(
                (r) => r.conflictId === conflict.itemId
              );
                    ...
                    <div className="flex items-center justify-between p-3 bg-blue-50 border border-blue-200 rounded-lg">
                      <div className="flex items-center space-x-3">
                        <span className="text-lg">
                          {getTrackIcon(conflict.suggestedTrack.id)}
                        </span>
                        <div>
                      ...
                      <button
                        onClick={() =>
                          handleTrackSelection(
                            conflict.itemId,
                            conflict.suggestedTrack,
                            false
                          )
                        }
                        className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${
                          resolution.selectedTrack.id ===
                            conflict.suggestedTrack.id &&
                          !resolution.userOverride
                          ...
                          {conflict.alternatives.map((alternative) => (
                            ...
                            >
                              ...
                              <button
                                ...
                                className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${
                                  resolution.selectedTrack.id ===
                                    alternative.id && resolution.userOverride
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {EDUCATIONAL_TRACKS.map((track) => (
                          <button
                            key={track.id}
                            onClick={() =>
                              handleTrackSelection(conflict.itemId, track, true)
                            }
        ...
        <div className="px-6 py-4">
          <div className="space-y-6">
            ...
            <div>
              ...
              <div className="space-y-3">
                {Object.entries(statistics.itemsByTrack).map(
                  ([trackName, count]) => (
                    <div
                      key={trackName}
              ...
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                <div className="flex items-center space-x-2">
                  ...
                  <div className="text-sm font-medium text-yellow-800">
                    {statistics.conflictCount} conflict
                    {statistics.conflictCount !== 1 ? 's' : ''} detected
                  </div>
                </div>
                <div className="text-sm text-yellow-700 mt-1">
                  Some items require manual track assignment and will need your
                  review.
                </div>
              </div>
            )}
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
              ...
              <ul className="text-sm text-blue-700 mt-2 space-y-1">
                <li>
                  • Timeline items will be automatically assigned to appropriate
                  educational tracks
                </li>
