...
export function VisualSettingsPanel({
  ...
  const handleResetToFactory = () => {
    if (
      confirm(
        'Are you sure you want to reset all visual settings to factory defaults? This cannot be undone.'
      )
    ) {
      try {
  ...
  const handleClearCustomSettings = () => {
    if (
      confirm(
        'Are you sure you want to clear all custom themes, gradients, and settings? This cannot be undone.'
      )
    ) {
      try {
  ...
  const [reduceMotion, setReduceMotion] = useState<boolean>(() => {
    try {
      return visualSettingsManager.getReduceMotion();
    } catch {
      return false;
    }
  });
      ...
      <div className="border border-border-subtle rounded-lg overflow-hidden">
        ...
        <div className="p-4 space-y-4">
          ...
          <div>
            ...
            <label className="flex items-center justify-between bg-background-tertiary border border-border-subtle rounded p-2">
              <div>
                <div className="text-sm text-text-primary">Reduce motion</div>
                <div className="text-xs text-text-secondary">
                  Honor system "prefers-reduced-motion" or force reduced motion
                  for subtle backgrounds and UI animations.
                </div>
              </div>
              <input
                type="checkbox"
                ...
                onChange={(e) => {
                  ...
                  try {
                    visualSettingsManager.setReduceMotion(e.target.checked);
                  } catch {
                    void 0; // ignore
                  }
                }}
