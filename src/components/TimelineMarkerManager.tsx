...
export function TimelineMarkerManager({ 
  ...
  }, [newRegionName, newRegionStart, newRegionEnd, currentProject, regions, timelineDuration, updateProject]);
  ...
  // Update region
  const updateRegion = useCallback((regionId: string, updates: Partial<TimelineRegion>) => {
    if (!currentProject) return;
  ...
  const jumpToRegion = useCallback((region: TimelineRegion) => {
  ...
  return (
    <div className={`timeline-marker-manager bg-gray-900 border-l border-gray-700 ${className}`}>
      {/* Header */}
            ...
            <div className="space-y-2">
              ...
              {markers.length === 0 && (
                <div className="text-center text-gray-500 py-4">
                  <p>No markers yet</p>
                  <p className="text-xs">Add markers to mark important timeline positions</p>
                </div>
              )}
            </div>
          )}
              ...
              <div className="grid grid-cols-2 gap-2 mb-2">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setNewRegionStart(parseFloat(e.target.value))}
                ...
                <div>
                  ...
                  <input
                    ...
                    step="0.1"
                    value={newRegionEnd}
                    onChange={(e) => setNewRegionEnd(parseFloat(e.target.value))}
                    className="w-full px-2 py-1 text-xs bg-gray-700 border border-gray-600 rounded text-white"
                  />
                </div>
              </div>
            ...
            <div className="space-y-2">
              {regions.map((region) => (
                <RegionItem
                  key={region.id}
                  region={region}
                  isActive={currentTime >= region.startTime && currentTime <= region.endTime}
              ...
              {regions.length === 0 && (
                <div className="text-center text-gray-500 py-4">
                  <p>No regions yet</p>
                  <p className="text-xs">Add regions to mark sections of your timeline</p>
                </div>
              )}
            </div>
          )}
...
function MarkerItem({ marker, isActive, onUpdate, onDelete, onJump, formatTime }: MarkerItemProps) {
...
function RegionItem({ region, isActive, onUpdate, onDelete, onJump, formatTime, maxTime }: RegionItemProps) {
  const [isEditing, setIsEditing] = useState(false);
  ...
  return (
    <div className={`p-2 rounded border transition-all ${
      isActive ? 'bg-green-900 border-green-600' : 'bg-gray-800 border-gray-700'
    }`}>
