    ...
    return (
      <div className={`flex items-center gap-2 ${className}`}>
        ...
        <div className="relative" ref={videoMenuRef}>
          ...
          {showVideoMenu && (
            <div className="absolute top-full left-0 mt-1 bg-gray-800 border border-gray-600 rounded-md shadow-lg z-50 min-w-[200px] overflow-hidden">
              <div className="py-1">
                {videoQuickActions.map((action) => (
                  <button
                    key={action.id}
                    onClick={action.action}
                    className="w-full text-left px-3 py-2 text-xs text-gray-200 hover:bg-gray-700 transition-colors flex items-center gap-2"
                  >
                    {action.icon}
                    <div>
