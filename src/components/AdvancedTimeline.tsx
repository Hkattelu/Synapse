...
import type { TimelineItem, MediaAsset, Keyframe, TrackGroup, TimelineMarker } from '../lib/types';
...
export function AdvancedTimeline({ className = '' }: AdvancedTimelineProps) {
  ...
  const addKeyframe = useCallback((property: string, time: number, value: any) => {
  ...
  const handleKeyframeClick = useCallback((e: React.MouseEvent, keyframeId: string, itemId: string) => {
    e.preventDefault();
    e.stopPropagation();
  ...
  const handleKeyframeDragStart = useCallback((e: React.MouseEvent, keyframe: Keyframe, itemId: string) => {
    e.preventDefault();
    e.stopPropagation();
  ...
  const handleTimelineDoubleClick = useCallback((e: React.MouseEvent) => {
    ...
    const time = Math.max(0, Math.min(clickTime - item.startTime, item.duration));
    addKeyframe('opacity', time, item.properties.opacity || 1);
  }, [selectedItems, timeline, ui.timeline.scrollPosition, pixelsToTime, addKeyframe]);
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent) => {
    ...
    if (dragState.dragType === 'keyframe' && dragState.keyframeId) {
      ...
      const newTime = snapToGrid(Math.max(0, dragState.startTime + deltaTime));

      const item = timeline.find(item => item.id === dragState.itemId);
      if (!item) return;

      const clampedTime = Math.min(newTime, item.duration);
      const updatedItem = KeyframeManager.updateKeyframe(item, dragState.keyframeId, { time: clampedTime });
      
      updateTimelineItem(dragState.itemId, { keyframes: updatedItem.keyframes });
    }
  }, [dragState, ui.timeline.scrollPosition, pixelsToTime, snapToGrid, timeline, updateTimelineItem]);
  ...
  const generateKeyframesFromPresets = useCallback((itemId: string) => {
    const item = timeline.find(item => item.id === itemId);
    if (!item) return;
  ...
  const deleteSelectedKeyframes = useCallback(() => {
    selectedKeyframes.forEach(keyframeId => {
      const item = timeline.find(item => item.keyframes.some(k => k.id === keyframeId));
      if (!item) return;
  ...
  return (
    <div className={`advanced-timeline bg-gray-800 border-t border-gray-700 ${className}`}>
      ...
      <div className="timeline-header bg-gray-900 border-b border-gray-700 p-2 flex items-center justify-between">
        <div className="flex items-center space-x-4">
          ...
          <div className="flex items-center space-x-2">
            <button
              ...
              className={`px-2 py-1 text-xs rounded ${
                showKeyframes ? 'bg-purple-600 text-white' : 'bg-gray-700 text-gray-300'
          ...
          <div className="flex items-center space-x-2">
            <button
              onClick={() => updateTimelineView({ zoom: Math.max(0.1, ui.timeline.zoom - 0.2) })}
            ...
            <button
              onClick={() => updateTimelineView({ zoom: Math.min(5, ui.timeline.zoom + 0.2) })}
              className="p-1 text-gray-400 hover:text-white transition-colors"
              title="Zoom In"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
interface AdvancedTimelineClipProps {
  ...
  onKeyframeClick: (e: React.MouseEvent, keyframeId: string, itemId: string) => void;
  onKeyframeDragStart: (e: React.MouseEvent, keyframe: Keyframe, itemId: string) => void;
  onItemSelect: () => void;
...
function AdvancedTimelineClip({ 
  ...
  return (
    ...
    >
      ...
      {/* Keyframes */}
      {showKeyframes && isSelected && item.keyframes.map((keyframe) => (
        <div
          key={keyframe.id}
          className={`
