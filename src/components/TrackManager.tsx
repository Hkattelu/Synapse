...
export function TrackManager({ className = '', onTrackGroupChange }: TrackManagerProps) {
  ...
  const trackStats = React.useMemo(() => {
    const stats: Record<number, { itemCount: number; duration: number; types: Set<string> }> = {};
  ...
  const maxTrack = Math.max(...timeline.map(item => item.track), ...trackGroups.flatMap(g => g.tracks), 3);
  ...
  }, [newGroupName, trackGroups, currentProject, updateProject, onTrackGroupChange]);

  // Add track to group
  const addTrackToGroup = useCallback((groupId: string, trackIndex: number) => {
  ...
  const removeTrackFromGroup = useCallback((groupId: string, trackIndex: number) => {
    const updatedGroups = trackGroups.map(group => 
      group.id === groupId 
  ...
  const updateTrackGroup = useCallback((groupId: string, updates: Partial<TrackGroup>) => {
    const updatedGroups = trackGroups.map(group => 
  ...
  // Delete group
  const deleteTrackGroup = useCallback((groupId: string) => {
  ...
  const selectTrackItems = useCallback((trackIndex: number) => {
    const trackItems = timeline.filter(item => item.track === trackIndex).map(item => item.id);
    selectTimelineItems(trackItems);
  }, [timeline, selectTimelineItems]);

  // Get items for a specific track
  const getTrackItems = useCallback((trackIndex: number) => {
    return timeline.filter(item => item.track === trackIndex);
  }, [timeline]);

  // Check if track is in any group
  const getTrackGroup = useCallback((trackIndex: number) => {
  ...
  return (
    <div className={`track-manager bg-gray-900 border-r border-gray-700 ${className}`}>
      {/* Header */}
      ...
      <div className="flex-1 overflow-y-auto">
        {trackGroups.length > 0 && (
          <div className="p-4 border-b border-gray-700">
            <h4 className="text-sm font-medium text-gray-400 mb-3">Track Groups</h4>
                  ...
                  <div className="p-3">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-2">
                        ...
                        <span className="text-xs text-gray-400">({group.tracks.length} tracks)</span>
                      </div>

                      <div className="flex items-center space-x-1">
                        {/* Visibility Toggle */}
                        <button
                          onClick={() => updateTrackGroup(group.id, { visible: !group.visible })}
                          className={`p-1 rounded transition-colors ${
                            group.visible ? 'text-blue-400 hover:text-blue-300' : 'text-gray-500 hover:text-gray-400'
                          }`}
                        ...
                        {/* Solo Toggle */}
                        <button
                          onClick={() => updateTrackGroup(group.id, { solo: !group.solo })}
                          className={`p-1 rounded transition-colors ${
                            group.solo ? 'text-yellow-400 hover:text-yellow-300' : 'text-gray-500 hover:text-gray-400'
                          }`}
                          title={group.solo ? 'Unsolo' : 'Solo'}
                        >
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.536 11.293l-1.414-1.414a1 1 0 00-1.414 0l-2.828 2.828a1 1 0 000 1.414l1.414 1.414a1 1 0 001.414 0l2.828-2.828a1 1 0 000-1.414z" />
                          </svg>
                        </button>

                        {/* Lock Toggle */}
                        <button
                          onClick={() => updateTrackGroup(group.id, { locked: !group.locked })}
                          className={`p-1 rounded transition-colors ${
                            group.locked ? 'text-red-400 hover:text-red-300' : 'text-gray-500 hover:text-gray-400'
                          }`}
                      ...
                      <div className="mt-3 pl-6 space-y-1">
                        {group.tracks.map((trackIndex) => {
                          ...
                          return (
                            ...
                            >
                              <div className="flex items-center space-x-2">
                                <span className="text-gray-300">Track {trackIndex + 1}</span>
                                {stats && (
                                  <>
                                    <div
                                    ...
                                    <span className="text-gray-400">
                                      {stats.itemCount} items • {Math.round(stats.duration * 10) / 10}s
                                    </span>
                                  </>
                                )}
                              </div>
                              ...
                              <div className="flex space-x-1">
                                ...
                                <button
                                  onClick={() => removeTrackFromGroup(group.id, trackIndex)}
        ...
        <div className="p-4">
          ...
          <div className="space-y-2">
            {Array.from({ length: maxTrack + 1 }).map((_, trackIndex) => {
              ...
              const isSelected = selectedItems.some(id => 
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-2">
                      <span className="text-sm text-white">Track {trackIndex + 1}</span>
                      {stats && (
                        <>
                          <div
                            className={`w-2 h-2 rounded ${getTrackTypeColor(stats.types)}`}
                  ...
                  {items.length === 0 && (
                    <div className="text-xs text-gray-500 mt-1">Empty track</div>
                  )}
                </div>
              );
            })}
