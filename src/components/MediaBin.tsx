...
export function MediaBin({ className = '' }: MediaBinProps) {
  ...
  const getMediaType = (mimeType: string): MediaAssetType | null => {
    ...
    if (SUPPORTED_FILE_TYPES.audio.includes(mimeType as unknown)) return 'audio';
    return null;
  };
  ...
  const generateThumbnail = async (file: File, type: MediaAssetType): Promise<string | undefined> => {
  ...
  const getMediaDuration = async (file: File, type: MediaAssetType): Promise<number | undefined> => {
    return new Promise((resolve) => {
      if (type === 'video') {
        const video = document.createElement('video');
        video.onloadedmetadata = () => {
  ...
  const handleDragStart = useCallback((event: React.DragEvent, asset: MediaAsset) => {
  ...
  return (
    <div className={`flex flex-col h-full bg-background-secondary ${className}`}>
      {/* Header */}
        ...
        {uploadErrors.length > 0 && (
          <div className="mt-3 space-y-1">
            {uploadErrors.map((error, index) => (
              <div key={index} className="text-xs text-status-error bg-status-error/10 p-2 rounded border border-status-error/20">
        ...
        {mediaAssets.length === 0 ? (
          ...
          >
            <div className="text-center p-8">
              ...
              <p className="text-text-primary font-medium mb-2">No Media Assets</p>
            ...
            <div className="grid grid-cols-2 gap-3">
              {mediaAssets.map((asset) => (
                ...
                >
                  ...
                  {/* Asset info */}
                  <div className="p-2">
                    <p className="text-sm font-medium text-text-primary truncate" title={asset.name}>
                      {asset.name}
                    </p>
