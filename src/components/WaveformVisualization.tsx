...
export function WaveformVisualization({
  ...
  useEffect(() => {
    ...
    drawWaveform(ctx, analysisData, width, height, color, currentTime, duration);
  ...
  }, [analysisData, width, height, color, currentTime, duration, syncPoints, showSyncPoints]);
  ...
  const handleSyncPointClick = useCallback((syncPoint: TimingSyncPoint) => {
    if (onSyncPointClick) {
  ...
  return (
    <div className={`relative ${className}`} style={{ width, height }}>
      ...
      {showSyncPoints && interactive && syncPoints.map((syncPoint) => {
...
function drawWaveform(
  ...
  if (duration > 0) {
    ...
    for (let i = Math.floor((playedWidth / width) * waveform.length); i >= 0; i--) {
      const x = (i / waveform.length) * width;
      const amplitude = waveform[i] * maxAmplitude;
      ctx.lineTo(x, centerY + amplitude);
    }
