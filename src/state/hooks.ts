...
export function useTimeline() {
  ...
  const moveTimelineItem = useCallback((id: string, startTime: number, track: number) => {
    dispatch({ type: 'MOVE_TIMELINE_ITEM', payload: { id, startTime, track } });
  }, [dispatch]);

  const resizeTimelineItem = useCallback((id: string, duration: number) => {
    dispatch({ type: 'RESIZE_TIMELINE_ITEM', payload: { id, duration } });
  }, [dispatch]);

  const selectTimelineItems = useCallback((ids: string[]) => {
  ...
  const getItemsAtTime = useCallback((time: number) => {
    return timeline.filter(item => 
      time >= item.startTime && time < item.startTime + item.duration
    );
  }, [timeline]);

  const getItemsByTrack = useCallback((track: number) => {
...
export function useMediaAssets() {
  ...
  const updateMediaAsset = useCallback((id: string, updates: Partial<MediaAsset>) => {
    dispatch({ type: 'UPDATE_MEDIA_ASSET', payload: { id, updates } });
  }, [dispatch]);

  const mediaAssets = state.project?.mediaAssets || [];

  const getMediaAssetById = useCallback((id: string) => {
    return mediaAssets.find(asset => asset.id === id);
  }, [mediaAssets]);

  const getMediaAssetsByType = useCallback((type: MediaAsset['type']) => {
...
export function useUI() {
  ...
  const setCurrentView = useCallback((view: 'dashboard' | 'studio') => {
  ...
  const updatePlaybackState = useCallback((updates: Partial<typeof state.ui.playback>) => {
    dispatch({ type: 'UPDATE_PLAYBACK_STATE', payload: updates });
  }, [dispatch]);

  const updateTimelineView = useCallback((updates: Partial<typeof state.ui.timeline>) => {
...
export function usePlayback() {
  ...
  const seek = useCallback((time: number) => {
    dispatch({ type: 'UPDATE_PLAYBACK_STATE', payload: { currentTime: time } });
  }, [dispatch]);
