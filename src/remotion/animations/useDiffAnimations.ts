  ...
  return useMemo(() => {
    ...
    switch (animation.preset) {
            ...
            .map((part, index) => {
              ...
              if (part.added) {
                ...
                const fadeProgress = Math.max(
                  0,
                  Math.min(1, (relativeFrame - startFrame) / fadeInFrames)
                );
                const highlightIntensity =
                  animation.highlightIntensity * fadeProgress;
                style = `opacity: ${fadeProgress}; background-color: rgba(74, 222, 128, ${highlightIntensity * 0.3});`;
              } else if (part.removed) {
                className = 'diff-removed';
                const fadeProgress = Math.max(
                  0,
                  Math.min(1, relativeFrame / fadeOutFrames)
                );
      ...
      case 'diffHighlight': {
        ...
        const pulsePhase = animation.pulseEffect
          ? Math.sin(relativeFrame * 0.2) * 0.5 + 0.5
          : 1;
          ...
          animatedHtml: diffParts
            .map((part, index) => {
              ...
              if (part.added) {
                ...
                style = `background-color: ${animation.highlightColor}${Math.floor(
                  highlightOpacity * 255
                )
                  .toString(16)
                  .padStart(2, '0')}; transition: background-color 0.3s ease;`;
              } else if (part.removed) {
                className = 'diff-removed';
      ...
      case 'typewriterDiff': {
        ...
        const result = diffParts
          .map((part, index) => {
            ...
            if (
              visibleChars < partLength &&
              charCount + visibleChars === totalCharsToShow
            ) {
              showCursor = true;
            }
